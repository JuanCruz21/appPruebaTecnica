...
export const indexContent = async (category_id:number) => {
    try{
        const response = await Api.get('/v1/content',{
            params: { category_id: category_id }, // Pasar parámetros en `params`
        });
...
export const updateContent = async (id: number,title:string,description:string,urldata:any,category_id:number) => {
    try{
        const formData = new FormData();
        formData.append("title", title);
        formData.append("description", description);
        ...
        const response = await Api.put(`/v1/content/${id}`,formData);
        const data =  response.data;
        if (response.status !== 200) {
            throw new Error(data.message);
        }
